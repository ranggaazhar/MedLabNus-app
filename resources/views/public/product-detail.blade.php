...
<head>
    ...
    <style>
        ...
        /* Image Zoom Effect */
        .image-zoom-container { overflow: hidden; cursor: zoom-in; }
...
<body class="w-full overflow-x-hidden bg-gray-50" x-data="productDetailApp()">
    ...
    {{-- LOGIC FIX: Cek apakah spesifikasi benar-benar ada dan BUKAN kalimat 'tidak tersedia' --}}
    @php
                    ...
                    <div class="p-8 lg:p-16 flex flex-col justify-center animate-fade-in-right delay-300">
                        ...
                        <div class="mb-8 animate-fade-in-up delay-700">
                            ...
                            <div class="min-h-[200px]">
                                ...
                                <div 
                                    x-show="activeTab === 'informasi'" 
                                    {{-- x-cloak hanya jika specs tampil --}}
                        ...
                        <div class="animate-fade-in-up delay-900">
                            ...
                            <div class="flex gap-3">
                                <a href="https://twitter.com/intent/tweet?text={{ urlencode($produk->nama_produk) }}&url={{ urlencode(url()->current()) }}" 
                                   target="_blank" class="w-11 h-11 bg-gray-900 rounded-full flex items-center justify-center hover:bg-[#B1252E] hover:scale-110 transition-all duration-300">
                                    <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12.53.02C13.84 0 15.14.01 16.44 0c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>
                                </a>
                                <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" 
                                   target="_blank" class="w-11 h-11 bg-[#0077B5] rounded-full flex items-center justify-center hover:scale-110 transition-all duration-300">
                                ...
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" 
                                   target="_blank" class="w-11 h-11 bg-[#1877F2] rounded-full flex items-center justify-center hover:scale-110 transition-all duration-300">
                                ...
                                <a href="https://wa.me/?text={{ urlencode($produk->nama_produk . ' - ' . url()->current()) }}" 
                                   target="_blank" class="w-11 h-11 bg-[#25D366] rounded-full flex items-center justify-center hover:scale-110 transition-all duration-300">
           ...
           {{-- Related Products --}}
            ...
            <div class="mt-20 animate-fade-in-up delay-1000">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <a href="{{ route('products.public') }}" class="text-[#B1252E] hover:underline font-semibold flex items-center gap-2 transition-colors duration-300 hover:text-[#8f1e26]">
                        Lihat Semua
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    class="group block bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                        ...
                        {{-- UBAH DISINI: bg-white jadi bg-gray-100 --}}
                        ...
                        <div class="px-5 pb-6 pt-4">
                            <p class="text-sm text-gray-500 mb-1 font-medium uppercase tracking-wider">{{ $related->pabrikan->nama_pabrikan ?? 'Unknown Brand' }}</p>
                            <h3 class="font-bold text-lg text-gray-900 leading-tight group-hover:text-[#B1252E] transition-colors duration-300 line-clamp-2">
                                {{ $related->nama_produk }}
                            </h3>
                        </div>
                    </a>
