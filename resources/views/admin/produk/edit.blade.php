...
@section('content')
    ...
    {{-- Form Pembungkus --}}
    <form id="productForm" action="{{ route('produk.update', $produk->produk_id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="kategori" id="kategori" required
                            ...
                            {{-- Opsi Kategori --}}
                            <option value="reagen" {{ old('kategori', $produk->kategori) == 'reagen' ? 'selected' : '' }}>
                                Reagen</option>
                            <option value="alat" {{ old('kategori', $produk->kategori) == 'alat' ? 'selected' : '' }}>
                                Alat</option>
                            <option value="steril" {{ old('kategori', $produk->kategori) == 'steril' ? 'selected' : '' }}>
                                Steril</option>
                            <option value="non steril"
                                {{ old('kategori', $produk->kategori) == 'non steril' ? 'selected' : '' }}>Non Steril
                            </option>
                            <option value="invitro"
                                {{ old('kategori', $produk->kategori) == 'invitro' ? 'selected' : '' }}>In Vitro</option>
                    ...
                    {{-- Baris Baru: Satuan, Harga Acuan, Stok Minimal (VERSI EDIT) --}}
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="harga_acuan" id="harga_acuan" placeholder="0"
                                value="{{ old('harga_acuan', $produk->harga_acuan) }}" required
                                class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-red-500 focus:ring-red-500 transition duration-150 shadow-sm">
                            @error('harga_acuan')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="stok_minimal" id="stok_minimal" placeholder="0"
                                value="{{ old('stok_minimal', $produk->stok_minimal) }}" required
                                class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-red-500 focus:ring-red-500 transition duration-150 shadow-sm">
                            @error('stok_minimal')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
            ...
            <div id="tab-spesifikasi" class="tab-content hidden">
                        ...
                        <div class="spec-item bg-gray-50 rounded-lg border border-gray-200 p-4">
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div class="flex justify-end">
                                    <button type="button"
                                        class="removeSpecBtn text-red-600 hover:text-red-800 transition-colors text-sm font-medium flex items-center gap-1">
                ...
                <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p class="text-sm text-blue-800">
                        ...
                        <strong>Tips:</strong> Tambahkan spesifikasi teknis seperti throughput, kapasitas, dimensi, berat,
                        voltase, dll.
                    </p>
                </div>
            </div>
...
@section('scripts')
    {{-- SweetAlert (Opsional, tapi disarankan) --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ==========================================
            // 1. AJAX Check Nama Produk (UPDATE MODE)
            // ==========================================
            let checkTimeout;
                ...
                namaProdukInput.addEventListener('input', function() {
                    ...
                    checkTimeout = setTimeout(() => {
                        fetch('{{ route('produk.checkNama') }}', {
                                ...
                                body: JSON.stringify({
                                    nama_produk: nama,
                                    produk_id: currentProdukId // Kirim ID produk ini untuk dikecualikan
                                })
                            })
            ...
            // ==========================================
            // Start index dari jumlah spesifikasi yang ada untuk menghindari duplikat index
            ...
            // Hapus Spesifikasi dengan SweetAlert
            document.addEventListener('click', function(e) {
                ...
                if (btn) {
                    ...
                    Swal.fire({
                        ...
                        text: "Item ini akan dihapus dari form.",
                        icon: 'warning',
            ...
            document.getElementById('productForm').addEventListener('submit', function(e) {
                ...
                if (!nama || !pabrikan || !kategori) {
                    e.preventDefault();
                    Swal.fire('Perhatian', 'Mohon lengkapi Nama Produk, Merk, dan Kategori', 'warning');
