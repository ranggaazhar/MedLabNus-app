        ...
        <div class="flex flex-col w-full">
            ...
            <div class="space-y-1 flex flex-col w-full">
                @php
                    ...
                    $isLogActive = request()->routeIs('log.*');
                    $isDashboardStrict = request()->routeIs('dashboard');
                    $isActiveDashboard =
                        $isDashboardStrict || (!$isProductActive && !$isPabrikanActive && !$isLogActive);
                @endphp
                ...
                <a href="{{ route('pabrikan.index') }}"
                    ...
                    <img src="{{ asset('icons/pabrik.svg') }}" alt="Pabrikan Icon"
                        class="w-6 h-6 object-contain flex-shrink-0 {{ $isPabrikanActive ? '' : 'opacity-50' }}">

                    <span class="block">Pabrikan</span>
                </a>

                {{-- 3. Link Activity Log (Audit Trail) --}}
                <a href="{{ route('log.index') }}"
                    class="!flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition-colors w-full
                    {{ $isLogActive ? 'bg-red-50 text-red-700' : 'text-gray-600 hover:bg-gray-100' }}">

                    {{-- Gunakan Icon History/Audit Trail --}}
                    <div class="w-6 h-6 flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-history text-lg {{ $isLogActive ? 'text-red-600' : 'text-gray-400' }}"></i>
                    </div>

                    <span class="block">Audit Trail</span>
                </a>
            </div>
        </div>
    </div>
